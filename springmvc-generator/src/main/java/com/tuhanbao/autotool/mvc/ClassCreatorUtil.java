...
import com.tuhanbao.autotool.mvc.clazz.IServiceClazzCreator;
import com.tuhanbao.autotool.mvc.clazz.MOClazzCreator;
import com.tuhanbao.autotool.mvc.clazz.ModelClazzCreator;
import com.tuhanbao.autotool.mvc.clazz.ServiceClazzCreator;
import com.tuhanbao.autotool.mvc.clazz.SolidClazzCreator;
import com.tuhanbao.autotool.mvc.clazz.SolidObject;
...
public class ClassCreatorUtil {
	...
	private static J2EEProjectInfo project = null;
	
	static {
	    String projectHead = ProjectConfig.PROJECT_HEAD;
	    if (StringUtil.isEmpty(projectHead)) {
	        project = new J2EEProjectInfo(ProjectConfig.PROJECT_URL, ProjectConfig.PROJECT_NAME);
	    }
	    else {
	        project = new J2EEProjectInfo(ProjectConfig.PROJECT_URL, ProjectConfig.PROJECT_NAME, projectHead);
	    }
	}
	
	public static void startCreateProject(Config solidConfig) {
		//读取所有配置文件
		initProjectByConfig(StringUtil.string2Array(ProjectConfig.CONFIG_URL));
		...
		try {
	        ...
	        SolidClazzCreator solidClazzCreator = new SolidClazzCreator(project);
	        
	        solidClazzCreator.writeSolidFiles(tables, getSolidObject(solidConfig));
...
//	        DBUtil.getSql(tables, dbType)
	...
	private static void generate(J2EEProjectInfo project, DBType dbType,
			J2EETable table) throws IOException {
		ClassInfo clazz = null;
		
		clazz = new ServiceClazzCreator(project).toClazz(table);
		String serviceUrl = getFullPath(project, clazz);
		if (!FileUtil.isExists(serviceUrl)) {
			TxtUtil.write(serviceUrl, clazz.toString());
		}

		clazz = new IServiceClazzCreator(project).toClazz(table);
		...
		//创建bean对象
        ClassInfo classInfo = new ModelClazzCreator(project).toClazz(table);
...
//		}
	...
	private static String getFullPath(J2EEProjectInfo project, ClassInfo clazz) {
	...
	public static void generateTableConstants(J2EEProjectInfo project) throws IOException {
		String url = project.getConstantsPath();
		String fullPath = project.getFullConstantsUrl();
     ...
     * @throws IOException
     */
    private static String getConstantClassStr(String url, Collection<J2EETable> tables, J2EEProjectInfo project) throws IOException
    {
...
//        sb.append("import com.tuhanbao.util.db.table.TableRegister;").append(Constants.ENTER);
	...
	private static void initProjectByConfig(String ... configUrl) {
	...
	private static void initConstants() throws IOException {
		...
		for (int i = 1; i < length; i++) {
			...
			if (!StringUtil.isEmpty(tableName)) {
				tableConfigs.put(arrays[i][0], arrays[i]);
			}
		}
...
//		}
        ...
        for (ClassInfo ci : classInfos) {
        	...
        	url = FileUtil.appendPath(project.getFullEnumUrl(),
        ...
        url = FileUtil.appendPath(project.getRootPath(), project.getConfigPath(),
	...
	private static void createProperties(J2EEProjectInfo project, String[][] arrays, String name) throws IOException {
		//生成Config文件
		...
		if (isDebugProperties) {
			TxtUtil.write(FileUtil.appendPath(project.getRootPath(), project.getConfigPath(), "debug",
					name + ".properties"), sb.toString());
		}
		else {
			//base不需要覆盖
			String url = FileUtil.appendPath(project.getRootPath(), project.getConfigPath(),
					name + ".properties");
...
//		T_CRAWLER_CLASS		NO	autocreate	
    ...
    private static void writeTableLanguage(J2EEProjectInfo project) throws IOException {
        StringBuilder sb = new StringBuilder();
